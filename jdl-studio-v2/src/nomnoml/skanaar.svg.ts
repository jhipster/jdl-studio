namespace nomnoml.skanaar {
  ...
  export function Svg(
      ...
      return {
        ...
        stroke: function () {
          ...
          this.attr.style =
            ...
            ";fill:none" +
            ";stroke-dasharray:" +
            lastDefined("dashArray") +
            ";stroke-width:" +
            lastDefined("strokeWidth") +
            ";";
          return this;
        },
        fill: function () {
          var base = this.attr.style || "";
          this.attr.style =
            base + "stroke:none; fill:" + lastDefined("fill") + ";";
          return this;
        },
    ...
    function tX(coord: number) {
      return Math.round(10 * trans(coord, "x")) / 10;
    }
    function tY(coord: number) {
    ...
    return {
      ...
      font: function (font) {
        ...
        if (waitingForFirstFont) {
          ...
          if (ctx) {
            ...
            canUseCanvas = /^(Arial|Helvetica|Times|Times New Roman)$/.test(
              primaryFont
            );
            if (canUseCanvas) {
              var fontSize =
                font.replace(/^.*font-size:/, "").replace(/;.*$/, "") + " ";
      ...
      arcTo: function (x1, y1, x2, y2) {
        last(elements).attr.d +=
          "L" + tX(x1) + " " + tY(y1) + " L" + tX(x2) + " " + tY(y2) + " ";
      },
      beginPath: function () {
      ...
      measureText: function (s: string) {
        if (canUseCanvas) {
          ...
          var italicSpec =
            (/\bitalic\b/.test(fontStr) ? "italic" : "normal") + " normal ";
      ...
      serialize: function (size: any, desc: string, title: string): string {
        function toAttr(obj: any) {
          function toKeyValue(key: string) {
            return key + '="' + obj[key] + '"';
          }
