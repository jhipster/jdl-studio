namespace nomnoml {
  ...
  export function layout(
    ...
    function measureLines(lines: string[], fontWeight: "normal" | "bold") {
      ...
      return {
        width: Math.round(
          Math.max(...lines.map(measurer.textWidth)) + 2 * config.padding
        ),
        height: Math.round(
    ...
    function layoutCompartment(
      c: Compartment,
      compartmentIndex: number,
      style: Style
    ) {
      var textSize = measureLines(
        c.lines,
        compartmentIndex ? "normal" : "bold"
      );
      ...
      g.edges().forEach(function (edgeObj) {
        ...
        layoutLabel(
          ...
          rel.startLabel,
          startP,
          adjustQuadrant(quadrant(startP, start, 4), start, end)
        );
        layoutLabel(
          // @ts-ignore
          rel.endLabel,
          endP,
          adjustQuadrant(quadrant(endP, end, 2), end, start)
        );
        left = Math.min(
          left,
          // @ts-ignore
      ...
      if (!label.text) {
        ...
        (label.x =
          ...
          (quadrant == 1 || quadrant == 4
            ? config.padding
            : -label.width - config.padding)),
          (label.y =
            point.y +
            (quadrant == 3 || quadrant == 4
              ? config.padding
              : -label.height - config.padding));
      }
    }
